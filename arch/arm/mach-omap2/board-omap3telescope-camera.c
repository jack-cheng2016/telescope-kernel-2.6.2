...
#define ISP_MT9P031_MCLK	48000000 //max 48Mhz

/* Arbitrary memory handling limit */
//#define MT9P031_BIGGEST_FRAME_BYTE_SIZE	PAGE_ALIGN((2592 * 1944) * 2 * 4 )
#define MT9P031_BIGGEST_FRAME_BYTE_SIZE	PAGE_ALIGN((1280 * 1024) * 1 * 4 )
...
static struct isp_interface_config mt9p031_if_config = {
	.ccdc_par_ser		= ISP_PARLL,
	 /*no shift add by cheng*/	
	.dataline_shift		= 0x1,
	.hsvs_syncdetect	= ISPCTRL_SYNC_DETECT_HSRISE,//ISPCTRL_SYNC_DETECT_VSRISE, // vs rising
	.strobe			= 0x0,
	.prestrobe		= 0x0,
	.shutter		= 0x0,
	//.cam_mclk		= ISP_MT9P031_MCLK,
	.pixelclk			= 24000000,//24MHZ
	.wenlog 		= ISPCCDC_CFG_WENLOG_AND,
	.wait_hs_vs		= 2,
	.u.par.par_bridge	= 0x0,
	.u.par.par_clk_pol	= 0x0,
...
static struct v4l2_ifparm mt9p031_ifparm_s = {
	...
	.u 	 = {
		.raw = {
			.frame_start_on_rising_vs = 1,//YES RIGHT
			.bt_sync_correct	= 0, //NO bt656
			.swap			= 0,//NO SWAP
			.latch_clk_inv		= 0,//???not sure
			.nobt_hs_inv		= 0,	/* active high */
			.nobt_vs_inv		= 0,	/* active high */
			.clock_min		= MT9P031_CLK_MIN, //6MHZ
			.clock_max		= MT9P031_CLK_MAX,//48MHZ
		},
	},
};
...
static int telescope_cam_probe(struct platform_device *pdev)
{
	unsigned int cnt=0;
	printk("telescope_cam_probe().....................\n");
	...
	gpio_direction_output(LEOPARD_RESET_GPIO, 1); //cheng,set default to 1
	...
	printk(KERN_INFO "omap3telescopelmb: Driver registration complete\n");

	return 0;
}

static int telescope_cam_remove(struct platform_device *pdev)
{
	if (regulator_is_enabled(cam_1v8_reg))
		regulator_disable(cam_1v8_reg);
	regulator_put(cam_1v8_reg);
...
static int telescope_cam_suspend(struct device *dev)
{
	return 0;
}

static int telescope_cam_resume(struct device *dev)
{
	return 0;
}

static struct dev_pm_ops telescope_cam_pm_ops = {
	.suspend = telescope_cam_suspend,
	.resume  = telescope_cam_resume,
};

static struct platform_driver telescope_cam_driver = {
	.probe		= telescope_cam_probe,
	.remove		= telescope_cam_remove,
	.driver		= {
		.name	= "telescope_cam",
		.pm	= &telescope_cam_pm_ops,
	},
};

/**
 * @brief omap3telescopelmb_init - module init function. Should be called before any
 *                          client driver init call
 *
 * @return result of operation - 0 is success
 */
int __init omap3telescopelmb_init(void)
{
	/* NOTE: Beagle xM boards are the only ones with camera interface */
	if (cpu_is_omap3630())
		platform_driver_register(&telescope_cam_driver);

	return 0;
}
late_initcall(omap3telescopelmb_init);
